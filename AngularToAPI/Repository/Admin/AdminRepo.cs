...
namespace AngularToAPI.Repository.Admin
{
    public class AdminRepo : IAdminRepository
    {
        ...
        {
            // Real path فى حالة عمل PUBLISH
            // var filepath = Path.Combine(_host.WebRootPath + "/images/actors/", img.FileName);
            var filePath = Path.Combine(@"D:\AangularTutorial\CinamaMovies\src\assets\images\actors", img.FileName);
           ...
           string catId, List<int> ids, string[] links)
        {
            ...
            var filePath = Path.Combine(@"D:\AangularTutorial\CinamaMovies\src\assets\images\posts", img.FileName);
            using (FileStream fileStream = new FileStream(filePath, FileMode.Create))
            {
                await img.CopyToAsync(fileStream);
            }
            ...
            filePath = Path.Combine(@"D:\AangularTutorial\CinamaMovies\src\assets\videos", video.FileName);
            using (FileStream fileStream = new FileStream(filePath, FileMode.Create))
            {
        ...
        public async Task<IEnumerable<RolePermission>> GetRolePermissionAsync(string RoleId)
        {
            if (RoleId == null)
            {
                return null;
            }
            //var role = await _db.RolePermissions.ToListAsync(x => x.RoleId == RoleId);
            var role = await _db.RolePermissions.Where(x => x.RoleId == RoleId).ToListAsync();
            if (role == null)
            {
                return null;
            }
        ...
        public async Task<RolePermission> GetRowRolePermission(string RoleId, int componentId)
        {
            if (RoleId == null || componentId < 0)
            {
                return null;
            }
            var Role = await _db.RolePermissions.FirstOrDefaultAsync(x => x.RoleId == RoleId && x.componentId == componentId);
            if (Role == null)
            {
                return null;
            }
        ...
        public Microsoft.AspNetCore.Mvc.IActionResult GetActorBy_Procedures()
        {
            //var userType = dbContext.Set().FromSql("dbo.SomeSproc @Id = {0}, @Name = {1}", 45, "Ada");
            var userType = _db.Set<Actor>().FromSqlRaw("GetActorBy_Procedures").ToList();
            return new Microsoft.AspNetCore.Mvc.ObjectResult(new { userType });
        }
        public async Task<IEnumerable<RolePermission>> GetRolePermission_ProceduresAsync(string RoleId)
        {
            if (RoleId == null)
            {
                return null;
            ...
            var role = await _db.Set<RolePermission>().FromSqlRaw("GetRolePermission_Procedure @RoleId", xparams).ToListAsync();
            if (role == null)
            {
                return null;
            }
