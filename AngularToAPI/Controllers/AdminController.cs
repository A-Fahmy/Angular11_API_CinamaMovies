...
namespace AngularToAPI.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<IEnumerable<RolePermission>> GetRolePermission(string RoleId)
        {
            if (RoleId == null || RoleId=="")
            {
                return null;
            }
            var role = await _repo.GetRolePermissionAsync(RoleId);
            if (role != null)
            {
                return role;
            }
        ...
        [Route("GetRowRolePermission/{RoleId}/{componentId}")]
        [HttpGet]
        public async Task<ActionResult<RolePermission>> GetRowRolePermission(string RoleId, int componentId)
        {
            if (RoleId == null || componentId < 0)
            {
                return null;
            }
            var row = await _repo.GetRowRolePermission(RoleId, componentId);
            if (row != null)
            {
                return row;
            }
        ...
        [HttpPost]
        public async Task<IEnumerable<RolePermission>> InsertRolePermission(IEnumerable<RolePermission> LtsrolePermissio)
        {

            if (LtsrolePermissio == null)
            {
                return null;
            }

            var role = await _repo.InsertRolePermissionAsync(LtsrolePermissio);
            if (role != null)
            {
                return role;
            }
        ...
        [Route("GetActorByProcedures")]
        [HttpGet]
        public IActionResult GetActorByProcedures()
        {
            var _Order = _repo.GetActorBy_Procedures();
            if (_Order != null)
            {
                return _Order;
            }
        ...
        [Route("GetRolePermission_Procedures/{RoleId}")]
        [HttpGet]
        public async Task<IEnumerable<RolePermission>> GetRolePermission_Procedures(string RoleId)
        {
            if (RoleId == null || RoleId == "")
            {
                return null;
            }
            var role = await _repo.GetRolePermission_ProceduresAsync(RoleId);
            if (role != null)
            {
                return role;
            }
